...
# Data Integration
# Merge the two DataFrames on the stock_symbol column.
df = pd.merge(df_stocks, df_companies, on='stock_symbol', how='left')

# Convert the 'date' column to datetime format
df['date'] = pd.to_datetime(df['date'])

# Groupby aggregation and summary statistics
investment_stats = df.groupby('stock_symbol').agg({
...
print("Null Values:", df_companies.isnull().sum())

# Outlier Detection and Treatment
# Calculate the Z-score for the 'adj_close' column
df['z_score'] = stats.zscore(df['adj_close'])
...
df_AAPL = df[df['stock_symbol'] == 'AAPL']
